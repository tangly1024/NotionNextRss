...
export async function getNotionPageData({ pageId, from }) {
  ...
  if (data && data.pageIds?.length > 0) {
    ...
    const pageIds = getAllPageIds(data.collectionQuery, data.collectionId, data.collectionView, data.viewIds)
    for (let i = 0; i < pageIds.length; i++) {
      const id = pageIds[i]
      const value = data.block[id]?.value
      if (!value) {
        continue
      }
      const properties = (await getPageProperties(id, data.block, data.schema, null, data.tagOptions, data.siteInfo)) || null
      if (properties) {
        collectionData.push(properties)
      }
    }

    data.allPosts = collectionData.filter(post => {
      return post.title
    })
